# BoringCache Benchmarks

This repo gathers JSON results from the standalone BoringCache benchmark repos.

## Benchmark Repos

- `boringcache/benchmark-hugo`
- `boringcache/benchmark-hugo-go`
- `boringcache/benchmark-immich`
- `boringcache/benchmark-mastodon`
- `boringcache/benchmark-posthog`
- `boringcache/benchmark-storybook`
- `boringcache/benchmark-opentelemetry-java`
- `boringcache/benchmark-spring-ai`
- `boringcache/benchmark-grpc`
- `boringcache/benchmark-zed`
- `boringcache/benchmark-n8n`

## Data Feed

`data/latest/` is regenerated by `.github/workflows/publish-index.yml`.

- `index.json`: latest complete same-commit GitHub Actions Cache and BoringCache pair
  for each benchmark lane
- `pairs.json`: selected commit-pair evidence
- `windows.json`: rolling summaries over the selected pair window
- `health.json`: per-lane flow health
- `providers.json`: per-provider lane samples for GitHub Actions Cache, BoringCache,
  Depot Cache, and BuildBuddy Cache where those workflows exist. Provider
  headlines use the benchmark artifact build row (`cold_seconds` for cold or
  commit builds, warm scenario seconds for warm reruns), never workflow wall
  time. Narrow build-only splits and raw workflow totals may remain in samples
  as diagnostic context only. Storage is reported only for providers with
  benchmark-artifact storage evidence.
- `benchmarks/*.json`: per-benchmark detail payloads
- `report.md`: small generated table for quick human review

## Refresh

```bash
mise exec -- ruby scripts/publish-index.rb
```

The scheduled workflow runs every four hours and commits `data/latest/`.

## Tests

```bash
mise exec -- ruby test/publish_index_test.rb
mise exec -- ruby test/benchmark_reporting_test.rb
mise exec -- ruby test/benchmark_registry_alignment_test.rb
mise exec -- ruby test/benchmark_workflow_guardrails_test.rb
mise exec -- ruby test/write_benchmark_artifacts_test.rb
```

## Guardrails

`.github/workflows/guardrails.yml` checks out the standalone benchmark repos
listed in `scripts/publish-index.rb` and runs the registry alignment plus
workflow guardrail tests. The workflow lint fails if Docker benchmark workflows
reintroduce Dockerfile-internal BoringCache helper wiring, local proxy probes, a
second `docker-layers` registry tag, or stop consuming the `boringcache/one`
Docker import-readiness outputs.
