"""Run EdgeZero's unchanged app-CLI build with credentials outside its container."""
...
def execute(command, extra=None, check=True, **kwargs):
    ...
    return subprocess.run(["docker", "exec", name, "/usr/bin/env", "-i",
                           *[k + "=" + v for k, v in environment.items()], *command],
                          check=check, **kwargs)
...
def prepare():
    ...
    run("docker", "run", "--detach", "--name", name, *isolation(), "--network", "host",
        *mounts([(workspace / "edgezero", "/opt/edgezero", True),
                 ...
                 (compiler, "/compiler-cache", False)]), IMAGE, "sleep", "infinity")
    execute(["bash", "-euc", """
...
def build(smoke=False):
    if os.environ["PROVIDER"] == "boringcache":
        ...
        cache = {"SCCACHE_DIR": "/compiler-cache", "SCCACHE_CACHE_SIZE": "10G",
                 "SCCACHE_SERVER_PORT": "4226", "SCCACHE_IDLE_TIMEOUT": "0"}
    if smoke:
        command = ["bash", "-euc", """
